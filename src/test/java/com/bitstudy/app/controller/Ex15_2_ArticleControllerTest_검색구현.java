...
class Ex15_2_ArticleControllerTest_검색구현 {
    ...
    public Ex15_2_ArticleControllerTest_검색구현(@Autowired MockMvc mvc) {
        this.mvc = mvc;
    }
    ...
    /* 1-2) 게시판(리스트) 페이지*/
    @Test
    @DisplayName("[view][GET] 게시글 리스트(게시판) 페이지 - 검색어와 함계 호출")
    public void givenSearchKeyword_thenReturnArticles() throws Exception {
        // Given
        SearchType searchType = SearchType.TITLE;
        /* 타임리프에서 index.html로 넣으면 enum타입 그대로 영어로 나온다.
        *  검색어가 있게 할건데 이번에는 제목으로만 테스트 할거임 */
        String searchValue = "title"; /* 키워드 */

        given(articleService.searchArticles(eq(searchType), eq(searchValue), any(Pageable.class))).willReturn(Page.empty());
        given(paginationService.getPaginationBarNumbers(anyInt(), anyInt())).willReturn(List.of(0,1,2,3,4));
        /* 페이징이 어떻게 나온던 일단 상관없고 단지 검색 했을때 페이징 기능을 호출하긴 하는가만 보면됨*/
        // When & Then
        mvc.perform(get("/articles")
        ...
        // queryParam 검색 쿼리 정하는( 타이틀과 검색어), .name() 없어도됨 안에있는 문구 가져옴
        // shift 두번 누르는 검색은 ctrl+f와 다른 방식의 검색(class, git 등으로 검색)
